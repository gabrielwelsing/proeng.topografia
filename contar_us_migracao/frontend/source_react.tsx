...
function SistemaLevantamento() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        try {
          ...
          if (userDoc.exists()) {
            setIsApproved(userDoc.data().status === 'approved');
          } else {
            // Admin always approved; others need approval
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'levantamentos'), orderBy('timestamp', 'desc'));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const list: ProjetoSalvo[] = [];
      snapshot.forEach((docSnap) => {
        const data = docSnap.data();
        list.push({
          id: docSnap.id as any,
          ...data,
          // Converter timestamp do Firebase para Data ISO e formatada
          dataIso: data.timestamp ? new Date(data.timestamp.seconds * 1000).toISOString().split('T')[0] : data.dataIso,
        } as any);
      });
      setHistorico(list);
    }, (error) => {
      console.error("Erro ao sincronizar levantamentos:", error);
    });
  ...
  const salvarProjeto = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'levantamentos'), novoData);
      // setHistorico e localStorage removidos pois o onSnapshot atualizará o estado automaticamente
      ...
      <div className="h-screen flex flex-col items-center justify-center bg-slate-50 p-6 text-center font-sans">
        <div className="bg-white p-12 rounded-[3rem] shadow-2xl max-w-md border border-amber-100">
          ...
          <h2 className="text-2xl font-black text-slate-800">
            {firebaseUser ? 'Aguardando Aprovação' : 'Acesso Restrito'}
          </h2>
          {firebaseUser && (
            <button
            ...
            <div className="flex flex-wrap gap-1 items-center">
              ...
              {CATEGORIAS_LISTA.map((cat) => (
                <button
                  ...
                  className={`text-[9px] font-bold px-1.5 py-0.5 rounded border leading-none transition-colors ${categoriasSelecionadas.includes(cat)
              ...
              <div className="flex flex-wrap gap-1 items-center">
                ...
                {TOPOGRAFOS_LISTA.map((nome) => (
                  <button
                    ...
                    className={`text-[9px] font-bold px-1.5 py-0.5 rounded border leading-none transition-colors ${topografoSelecionado === nome
                ...
                <div className="flex flex-col items-center">
                  ...
                  <div className="flex gap-1 mt-0.5">
                    <button
                      ...
                      className={`text-[9px] font-bold px-1.5 py-0.5 rounded border leading-none transition-colors ${ambiental === 'SIM'
                    ...
                    <button
                      ...
                      className={`text-[9px] font-bold px-1.5 py-0.5 rounded border leading-none transition-colors ${ambiental === 'NÃO'
                  ...
                  <div className="flex gap-1 mt-0.5">
                    {['SST', 'SSC', 'SSTC'].map((sigla) => (
                      <button
                        ...
                        className={`text-[9px] font-bold px-1.5 py-0.5 rounded border leading-none transition-colors ${servidao === sigla
        ...
        <div className="flex flex-col items-end gap-2 shrink-0 pl-2">
          <div className="flex gap-2">
            <button
              ...
              className="text-[9px] font-bold text-blue-600 border border-blue-200 px-2 py-1 rounded hover:bg-blue-50 transition-colors"
            >
              Voltar ao Portal
            </button>
            <button
              onClick={() => signOut(auth)}
              className="text-[9px] font-bold text-red-400 border border-red-200 px-2 py-1 rounded hover:bg-red-50 transition-colors"
            >
              Sair
            </button>
          </div>
          ...
          <>
            <div className="flex-1 bg-slate-200 flex items-center justify-center relative overflow-hidden p-2">
              ...
              <button
                ...
                className="absolute top-4 left-4 bg-white px-3 py-1 rounded shadow text-[10px] text-slate-700 font-bold border hover:bg-slate-50 z-50"
              >
                ← VOLTAR
              </button>
            ...
            <aside className="w-56 bg-white border-l flex flex-col z-20 shrink-0 shadow-sm">
              <div className="p-3 bg-slate-50 border-b flex flex-col">
                ...
                {ambiental && (
                  <span
                    className={`text-[9px] font-bold mt-1 px-1 rounded w-fit ${ambiental === 'SIM'
                ...
                {paginas[paginaAtual]?.postes.map((p, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-[9px] font-bold uppercase ${p.tipo === 'existente'
                          ? 'text-orange-500'
                          : p.tipo === 'rural'
                            ? 'text-blue-500'
                            : 'text-emerald-500'
...
export default SistemaLevantamento;
